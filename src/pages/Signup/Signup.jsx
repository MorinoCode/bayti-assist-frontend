...
import signupLogo from '../../assets/loginpage/signup.png';
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          countryCode: signupMethod === 'phone' ? countryCode : undefined,
          password,
          role: activeTab,
          method: signupMethod,
        }),
      ...
      // Always redirect to role-specific onboarding after signup
      ...
      if (role === 'employer') {
        ...
        // Fallback or debug
        navigate(data.role === 'employer' ? '/onboarding-employee' : '/onboarding-worker');
      }
    } catch (error) {
      alert(error.message);
    } finally {
      ...
      <div className="login-card glass-card">
        <div className="login-header">
          <div className="logo-icon">
            <img src={signupLogo} alt="Signup" className="login-logo-img" />
          </div>
          <h1 className="brand-name">{t('brand')}</h1>
          <p className="subtitle">{t('create_account')}</p>
        </div>
        ...
        <div className="login-footer">
          <p>{t('already_have_account')} <Link to="/login">{t('sign_in_link')}</Link></p>
        </div>
      </div>
    </div>
  );
